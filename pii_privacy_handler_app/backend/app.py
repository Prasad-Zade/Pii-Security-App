...
def handle_messages(session_id):
    ...
    try:
        ...
        print("\n" + "="*80)
        print(f"[STEP 1] RECEIVED USER INPUT")
        ...
        if pii_analysis:
            print(f"\n[STEP 2] PII ANALYSIS FROM FRONTEND")
            print(f"  Masked Query: {pii_analysis.get('maskedQuery', 'N/A')}")
            print(f"  Detected Entities: {len(pii_analysis.get('allEntities', []))}")
            print(f"  Non-Dependent PII: {[e.get('value') for e in pii_analysis.get('nonDependentEntities', [])]}")
        ...
        print(f"\n[STEP 3] GEMINI API RESPONSE (RAW)")
        print(f"  LLM Response: {result.get('llm_response', 'No response')[:100]}...")
        
        print(f"\n[STEP 4] RECONSTRUCTION")
        print(f"  Final Response: {result.get('final_response', 'No response')[:100]}...")
        print(f"  Privacy Score: {result.get('privacy_score', 0.0)}")
        print("="*80)
        
        # Manual reconstruction if needed
        llm_response = result.get('llm_response', 'No response')
        final_response = result.get('final_response', llm_response)
        
        # Always try manual reconstruction for names
        if pii_analysis:
            print(f"[DEBUG] PII Analysis: {pii_analysis}")
            non_dependent_entities = pii_analysis.get('nonDependentEntities', [])
            print(f"[DEBUG] Non-dependent entities: {non_dependent_entities}")
            for entity in non_dependent_entities:
                print(f"[DEBUG] Processing entity: {entity}")
                if entity.get('type') == 'name':
                    original_name = entity.get('value', '')
                    print(f"[DEBUG] Original name: {original_name}")
                    print(f"[DEBUG] Final response before fix: {final_response[:100]}...")
                    if original_name:
                        fake_names = ['John', 'Alice', 'Bob', 'Mike', 'Emma', 'Jane', 'David', 'Lisa', 'Tom']
                        for fake_name in fake_names:
                            if fake_name in final_response and fake_name != original_name:
                                final_response = final_response.replace(fake_name, original_name)
                                print(f"[MANUAL FIX] Replaced {fake_name} with {original_name}")
                                break
                        print(f"[DEBUG] Final response after fix: {final_response[:100]}...")
        ...
        print(f"\n[STEP 5] SENDING TO FLUTTER APP")
        print(f"  bot_response: {message.get('bot_response', '')[:80]}...")
        print(f"  reconstructed_text: {message.get('reconstructed_text', '')[:80]}...")
        print(f"  privacy_score: {message.get('privacy_score', 0.0)}")
        print(f"  Reconstruction worked: {message.get('bot_response', '') != message.get('reconstructed_text', '')}")
        print("="*80 + "\n")
